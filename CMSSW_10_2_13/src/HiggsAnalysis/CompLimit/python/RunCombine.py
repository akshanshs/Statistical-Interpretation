...
for inputname in inputflist:
    massPoint = inputname.split('datacard')[-1].split('LT30cm.txt')[0]
    command = 'combine -M AsymptoticLimits '+inputname+' --name '+ massPoint+'_30cm'+' --noFitAsimov' #' --run both' #+' --run blind'
    print command
    os.system(command)
#    pause()
